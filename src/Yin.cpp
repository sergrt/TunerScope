 ...
 */
void YinPitchCalculator::Yin_init(Yin *yin, int16_t bufferSize, float threshold){
...
void YinPitchCalculator::Yin_free(Yin *yin){
    if (yin->yinBuffer) {
        free(yin->yinBuffer);
        yin->yinBuffer = nullptr;
    }
}
 ...
 */
float YinPitchCalculator::Yin_getProbability(Yin *yin){
    return yin->probability;
}

void YinPitchCalculator::Yin_difference(Yin *yin, float* buffer){
    int16_t i;
    int16_t tau;
    float delta;
 ...
 */
void YinPitchCalculator::Yin_cumulativeMeanNormalizedDifference(Yin *yin){
    int16_t tau;
    float runningSum = 0;
    yin->yinBuffer[0] = 1;
 ...
 */
int16_t YinPitchCalculator::Yin_absoluteThreshold(Yin *yin){
    int16_t tau;
 ...
 * shift value.
 */
float YinPitchCalculator::Yin_parabolicInterpolation(Yin *yin, int16_t tauEstimate) {
    float betterTau;
    int16_t x0;
    int16_t x2;
